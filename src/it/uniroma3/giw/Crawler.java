...
public class Crawler {
	...
	public void doCrawling() {
		...
		try {
			...
			this.addToPageToVisit(firstPage.getAnchors(), firstPage);
	...
	private void addToPageToVisit(List<HtmlAnchor> newAnchors, HtmlPage htmlPage) throws MalformedURLException {
		for(HtmlAnchor anchor : newAnchors) {
			if(!this.visitedPage.contains(HtmlAnchor.getTargetUrl(anchor.getHrefAttribute(), htmlPage).toString())) {
				System.out.println("aggiungo: "+HtmlAnchor.getTargetUrl(anchor.getHrefAttribute(), htmlPage)+" a "+anchor.getHrefAttribute());
				this.pageToVisit.add(anchor);
				this.visitedPage.add(HtmlAnchor.getTargetUrl(anchor.getHrefAttribute(), htmlPage).toString());
			}
		}
	}
